  ...
  for (const rule of rules as AlertRule[]) {
    try {
      ...
      if (rule.metric === 'drop_off') {
        ...
        const overallAvg =
          scores.length > 0 ? scores.reduce((acc, s) => acc + s.avg_score, 0) / scores.length : 10;
      ...
      if (breached) {
        ...
        console.log(
          `[alerts] Alert fired for tenant ${tenantId}, rule ${rule.id}: ${alertMessage}`,
        );
      }
    } catch (err) {
...
async function fireNotification(tenantId: string, rule: AlertRule, message: string) {
  ...
  if (rule.channel === 'slack' && slackUrl) {
    try {
      ...
      console.error(
        `[alerts] Failed to send Slack notification for tenant ${tenantId}:`,
