...
describe('evaluateAlerts', () => {
  ...
  it('should fire a Slack notification when drop-off threshold is breached', async () => {
    ...
    vi.spyOn(journeyLoader, 'listJourneys').mockResolvedValue([{ id: 'j1', title: 'Journey 1' }] as any);
  ...
  it('should fire an email notification when idle-user threshold is breached', async () => {
    ...
    vi.spyOn(metrics, 'getStuckUsers').mockResolvedValue([
      { userId: 'u1' }, { userId: 'u2' }, { userId: 'u3' }, { userId: 'u4' }, { userId: 'u5' }, { userId: 'u6' }
    ] as any);
    vi.spyOn(notifications, 'getNotificationConfig').mockResolvedValue({ email_to: 'admin@test.com' } as any);
